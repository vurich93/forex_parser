...
class BaseParser(BaseMeta):
    ...
    def _get_page(self, url):
        if hasattr(self, "page"):
            if self.page < 1:
                raise ValueError("Page is very small!")
...
class NewsParser(BaseParser):
    ...
    def get_news(self):
        try:
            ...
            if box is not None:
                self.news["head"] = box.find("h1", attr={"class": "articleHeader"}).text
                box_date = box.find("div", attr={"class": "contentSectionDetails"})
                news_date = box_date.find("span").text


if __name__ == "__main__":
    parser = Preview(page=10)
    parser.get_links()
